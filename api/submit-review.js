...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const thankYouEmail = resend.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:0;background:#fafaf7;">
          ...
          <div style="background:#fff;padding:36px 32px;border-left:1px solid rgba(13,13,13,.08);border-right:1px solid rgba(13,13,13,.08);">
            <p style="color:#0d0d0d;font-size:15px;line-height:1.72;margin:0 0 20px;">We've received your ${stars} rating for <strong>${safeService}</strong> and will review it shortly. Your feedback helps us improve and helps others make confident decisions about their projects.</p>
            <div style="background:#fafaf7;border-left:3px solid #c9a327;padding:16px 20px;border-radius:0 6px 6px 0;margin-bottom:24px;">
              <p style="color:#44504a;font-size:12px;text-transform:uppercase;letter-spacing:.1em;margin:0 0 8px;">Your review</p>
              <p style="color:#0d0d0d;font-size:14px;line-height:1.65;margin:0;font-style:italic;">"${safeMsg}"</p>
            </div>
            <p style="color:#44504a;font-size:14px;line-height:1.65;margin:0;">If you have any questions or need assistance with a new project, feel free to reply to this email — we'd love to hear from you again.</p>
          </div>
    ...
    await Promise.all([adminEmail, thankYouEmail]);
